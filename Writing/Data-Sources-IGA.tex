...
\citep{Lubbers2012}. Third, obtaining these data is time-consuming.
...
networks to understand political outcomes. New data sources make it
...
there are exciting opportunities to study who writes the rules and how
it matters.
...
published after 1994 and optical versions form 1939-1993.\footnote{See
...
scholars must overcome.\footnote{The raw texts of rules are not on
  regulations.gov or the independent agencies themselves. Because rules
  are usually published as PDFs, converting them to .txt for analysis
  introduces errors into the text.} First, the standard path from notice
of proposed rulemaking (NPRM) to public comments to a final rule is not
always straightforward. Some rules are withdrawn before a final rule.
Other agencies issue interim final rules subject change. For studies
that seek to compare the proposed and final rules, the most problematic
rules are the ones where one proposed rule gets broken up into a few
smaller final rules or the reverse, where small rules become bundled
into one final rule. These rules pose challenges for inference as the
processes that lead to amalgamation or separation may not be well
understood. Second, scholars must determine how to preprocess the
rulemaking data before feeding these data to a text analysis algorithm.
